...
### 1.1 Nền tảng lý thuyết & thuật ngữ

Để tiện theo dõi các phần sau, dưới đây là các khái niệm cốt lõi (định nghĩa trước khi dùng):

- **Trò chơi tổng bằng không (zero‑sum):** lợi ích hai người luôn cộng lại bằng 0 — một
  người thắng đúng bằng phần người kia thua. Liar's Dice 2 người là zero‑sum.
- **Thông tin không hoàn hảo (imperfect information):** mỗi người chỉ thấy xúc xắc của
  mình, không thấy tay đối thủ — khác cờ vua (thông tin hoàn hảo).
- **Tập thông tin (information set / infoset):** tập các trạng thái mà người đang đi
  **không phân biệt được** từ những gì họ quan sát. Ở đây một infoset = (tay mình, số xúc
  xắc đối thủ, cược đang đứng). Chiến lược được định nghĩa trên infoset, không phải trên
  trạng thái thật.
- **Chiến lược (strategy) σ:** phân phối xác suất trên các hành động hợp lệ tại mỗi infoset.
- **Cân bằng Nash (Nash equilibrium):** cặp chiến lược mà không ai đổi đơn phương để được
  lợi hơn. Trong trò chơi zero‑sum, chiến lược Nash là **tối ưu theo nghĩa minimax**: bảo
  đảm kết quả ≥ giá trị trò chơi (ở đây = 50% trong game đối xứng) trước **mọi** đối thủ.
- **Đáp trả tốt nhất (best response):** chiến lược tối đa hoá lợi ích trước một đối thủ cố
  định cho trước.
- **Khả năng bị khai thác (exploitability):** mức một chiến lược thua so với Nash khi gặp
  best‑response của đối thủ. Exploitability = 0 ⇔ chiến lược là Nash. Đây là thước đo
  "độ tốt" của một chiến lược trong trò chơi zero‑sum.
- **Hối hận (regret):** với một hành động `a` tại infoset `I`, regret đo "lẽ ra chọn `a`
  thì tốt hơn bao nhiêu so với cái đã chơi". **Regret matching** chọn hành động tỷ lệ với
  regret dương tích luỹ; tổng regret trung bình theo thời gian là **cận trên của
  exploitability** ⇒ regret → 0 thì chiến lược trung bình hội tụ về Nash (cơ sở của CFR).
...
**Sơ đồ luồng một lượt chơi** (vòng lặp do `Referee` điều phối):
...
1. Zinkevich, M., Johanson, M., Bowling, M., & Piccione, C. (2007). *Regret Minimization in
   Games with Incomplete Information.* NeurIPS 20. — thuật toán CFR gốc.
2. Tammelin, O. (2014). *Solving Large Imperfect Information Games Using CFR+.* arXiv:1407.5042.
   — biến thể CFR+ (regret matching+ + linear averaging) dùng trong dự án.
3. Lanctot, M., Waugh, K., Zinkevich, M., & Bowling, M. (2009). *Monte Carlo Sampling for
   Regret Minimization in Extensive Games.* NeurIPS 22. — MCCFR (hướng phát triển §7).
4. Brown, N., & Sandholm, T. (2018). *Superhuman AI for heads‑up no‑limit poker: Libratus.*
   Science 359(6374). — ứng dụng CFR quy mô lớn cho thông tin không hoàn hảo.
5. Russell, S., & Norvig, P. (2021). *Artificial Intelligence: A Modern Approach* (4th ed.).
   — tìm kiếm đối kháng/bất định (Ch. 5), ra quyết định dưới bất định (Ch. 16).
6. Gelman, A. et al. (2013). *Bayesian Data Analysis* (3rd ed.). — liên hợp Beta‑Bernoulli
   (cơ sở BayesianAgent).
...
```
> CFRAgent **nạp weights đã train nặng** từ `experiments/cfr_heavy.weights.json.gz` nếu có
> (mạnh ngay, không cần train lại); không có file thì tự train `--cfr-iters` vòng (mặc định
> 40k) — tăng số này (~50k) để đạt sức mạnh trong §5.1. Bản weights **đã nén gzip (~7MB) ĐƯỢC
> commit kèm repo** (`main.py`/`load_weights` tự đọc đuôi `.gz`), nên clone về là chạy CFR
> mạnh ngay. Bản `.json` chưa nén (~51MB) **không** được track (xem `.gitignore`); sinh lại
> bằng `agent.train(50000)` rồi `agent.save_weights("...weights.json.gz")` (lưu thẳng ra `.gz`).
